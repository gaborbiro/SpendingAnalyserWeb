		...
		try {
			...
			for (int i = 1; i <= reader.getNumberOfPages(); i++) {
				...
				if (finderBrought.getCount() == 1 && finderCarried.getCount() == 1
					...
					if (table.size() > 0) {
						System.out.println("\n\n\n\n\n");
						RowFinder finderPaimentType = parser.processContent(i,
								new RowFinder("Payment type and details"));
						...
						table.collapse(finderBrought.getLlx(0), finderPaidOut.getLlx(0) - moneyColumnBuffer - 1,
								new Grid.Collapse<String>() {

									public String collapse(List<String> items) {
										return items.stream().collect(Collectors.joining(" "));
									}
								});
						table.collapse(finderPaidOut.getLlx(0) - moneyColumnBuffer,
								finderPaidOut.getLlx(0) + finderPaidOut.getWidth(0), new Grid.Collapse<String>() {
						...
						table.collapse(finderPaidOut.getLlx(0) + finderPaidOut.getWidth(0) + 1,
								finderPaidIn.getLlx(0) + finderPaidIn.getWidth(0), new Grid.Collapse<String>() {
						...
						for (Iterator<Iterator<Cell<Float, String>>> rowI = table.iterator(); rowI.hasNext();) {
							...
							if (amount != null) {
								TransactionItem transactionItem = new TransactionItem(currentDate, "H " + description, amount);
